...
export const StatusPanel: React.FC<StatusPanelProps> = ({ selectedStage, onClose }) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl backdrop-blur-md flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
      
      {/* Panel Header */}
      <div className="flex items-center justify-between border-b border-slate-800 pb-3.5">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-slate-800 text-sky-400 border border-slate-700/60 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-950/70 p-3.5 rounded-xl border border-slate-800">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-4 text-xs font-mono">
          <div className="flex items-center gap-1.5 text-slate-300">
            ...
            <span className="text-slate-500 text-[11px]">evt/s</span>
          </div>
          <div className="flex items-center gap-1.5 text-slate-300">
      ...
      {/* Description */}
      <div>
        <h4 className="text-[11px] font-mono uppercase tracking-wider text-slate-400 font-semibold mb-1">
          Description
        </h4>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
            {selectedStage.specs.topicOrTable && (
              <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-800 flex flex-col">
                <span className="text-[10px] text-slate-500 uppercase">Target Stream / Table</span>
                <span className="text-slate-200 font-semibold truncate mt-0.5">
                  {selectedStage.specs.topicOrTable}
                </span>
              </div>
            )}
            {selectedStage.specs.engineVersion && (
              <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-800 flex flex-col">
                <span className="text-[10px] text-slate-500 uppercase">Engine Version</span>
                <span className="text-slate-200 font-semibold truncate mt-0.5">
                  {selectedStage.specs.engineVersion}
                </span>
              </div>
            ...
            {selectedStage.specs.clusterOrCatalog && (
              <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-800 flex flex-col">
                <span className="text-[10px] text-slate-500 uppercase">Cluster / Catalog URI</span>
                <span className="text-slate-200 font-semibold truncate mt-0.5">
                  {selectedStage.specs.clusterOrCatalog}
                </span>
              </div>
            ...
            {selectedStage.specs.throughputTarget && (
              <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-800 flex flex-col">
                <span className="text-[10px] text-slate-500 uppercase">Target SLA / Throughput</span>
                <span className="text-slate-200 font-semibold truncate mt-0.5">
                  {selectedStage.specs.throughputTarget}
                </span>
              </div>
      ...
      <div className="border-t border-slate-800 pt-3">
        <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 mb-1.5">
          ...
          <span className="text-sky-400/80 text-[10px]">GET /api/pipeline/status</span>
        </div>
        <pre className="bg-slate-950 p-2.5 rounded-lg text-[11px] font-mono text-slate-300 border border-slate-800 overflow-x-auto">
{JSON.stringify(
  {
    [selectedStage.id]: {
