...
export const PipelineStats: React.FC<PipelineStatsProps> = ({ summary }) => {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-md backdrop-blur-sm relative overflow-hidden group hover:border-slate-700 transition-all">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-md backdrop-blur-sm relative overflow-hidden group hover:border-slate-700 transition-all">
        ...
        <div className="mt-2 flex items-baseline gap-2">
          ...
          <span className={`text-xs font-mono font-semibold ${isAllHealthy ? 'text-emerald-400' : 'text-amber-400'}`}>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-md backdrop-blur-sm relative overflow-hidden group hover:border-slate-700 transition-all">
        ...
        <div className="mt-2 flex items-baseline gap-2">
          ...
          <span className="text-xs font-mono text-slate-400 font-medium">ms</span>
        </div>
        <p className="text-[11px] text-slate-400 mt-1">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-4 shadow-md backdrop-blur-sm relative overflow-hidden group hover:border-slate-700 transition-all">
        ...
        <div className="mt-2 flex items-baseline gap-2">
          <span className={`text-2xl font-mono font-bold tracking-tight ${
            summary.activeAlertCount === 0 ? 'text-white' : 'text-rose-400'
          }`}>
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-0.5 opacity-60 group-hover:opacity-100 transition-opacity ${
          summary.activeAlertCount === 0 ? 'bg-gradient-to-r from-slate-600 to-slate-700' : 'bg-gradient-to-r from-rose-500 to-amber-500'
        }`} />
      </div>
    </div>
  );
